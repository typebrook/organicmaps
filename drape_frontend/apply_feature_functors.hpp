...
protected:
  ...
  double PriorityToDepth(int const priority, drule::rule_type_t const ruleType, double const areaDepth) const;
...
public:
  ...
  void ProcessPointRule(drule::BaseRule const * rule, bool const isHouseNumber);
...
public:
  ...
  void ProcessAreaRule(drule::BaseRule const * rule, double const areaDepth, bool const isHatching);
