...
bool InitStylist(FeatureType & f, int8_t deviceLang, int const zoomLevel, bool buildings3d, Stylist & s)
{
  ...
  if (aggregator.m_mainCaptionFound)
  {
    s.m_captionDescriptor.Init(f, deviceLang, zoomLevel, geomType, aggregator.m_auxCaptionFound);
