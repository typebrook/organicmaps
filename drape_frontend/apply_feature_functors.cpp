...
double BaseApplyFeature::PriorityToDepth(int const priority, drule::rule_type_t const ruleType, double const areaDepth) const
{
  ...
  if (ruleType == drule::area || ruleType == drule::line)
  {
    if (depth < drule::kBasePriorityBgTop)
    {
      ...
      ASSERT(!std::isnan(areaDepth), (m_f.GetID()));
      depth = areaDepth;
    }
...
void ApplyPointFeature::ProcessPointRule(drule::BaseRule const * rule, bool const isHouseNumber)
{
...
void ApplyAreaFeature::ProcessAreaRule(drule::BaseRule const * rule, double const areaDepth, bool const isHatching)
{
...
void ApplyLineFeatureGeometry::ProcessLineRule(drule::BaseRule const * rule)
{
  ...
  double depth = PriorityToDepth(pLineRule->priority(), drule::line, 0);
...
{
#ifdef LINES_GENERATION_CALC_FILTERED_POINTS
  LinesStat::Get().InsertLine(m_f.GetID(), m_currentScaleGtoP, m_readCount, static_cast<int>(m_spline->GetSize()));
#endif
}
